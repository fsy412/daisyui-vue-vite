<template>
	<div
		class="p-1 tabs text-gray-300 md:w-1/5 2xl:w-[15%] flex flex-col justify-start"
	>
		<div class="flex w-full flex-col">
			<span class="text-left">Create Order</span>
			<div class="tabs tabs-boxed bg-neutral text-gray-200">
				<a
					class="tab tab-active text-gray-200 w-1/2"
					@click="onSideClick('buy')"
					ref="buy"
					>Buy</a
				>
				<a
					class="tab text-gray-200 w-1/2"
					@click="onSideClick('sell')"
					ref="sell"
					>Sell</a
				>
			</div>
			...
			<div v-if="store.state.orderType == 'limit'">
				<div class="form-control rounded-none bg-neutral mt-2">
					<div class="input-group">
						<span
							class="rounded-none bg-neutral text-gray-300 w-[80px]"
							>Price</span
						>
						<input
							type="text"
							placeholder="0.0"
							class="input input-bordered h-8 w-28 rounded-none bg-neutral"
							v-model="price"
						/>
		...
		<div class="mt-5 w-full px-2">
			<button class="w-full btn bg-green-500" @click="onPlaceOrder">
				Place Order
			</button>
		</div>
	</div>
